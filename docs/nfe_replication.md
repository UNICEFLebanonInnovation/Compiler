...
| `mscc.Registration` (+ child, services checklist, education history) | `mscc.registration` | `mscc.Registration` (+ `Child`, `ProvidedServices`, `EducationHistory`) |
...
their own, because BMA-NFE never generates them itself — it only reads them
to draw the child profile: `child.Child`, the services checklist
(`ProvidedServices`) and `EducationHistory`. The checklist is replaced as a
set on every push, mirroring `regenerate_services`; its `service_id` is sent
as a natural key (which service, which id) since it is a local primary key on
both sides, and saving any checklist-linked service republishes the
registration so the completed item arrives right behind it.

Scope is the whole MSCC module: every record, every partner, every round.

Not replicated, on purpose: the CLM/Dirasa attendance tables (not MSCC), the
administrative geography (`Location` — master data both systems already hold,
matched by P-code), and the Compiler's other assessment sheets
(`EducationProgrammeWLAssessment`, `EducationProgrammeSummerRSAssessment`,
`TarlAssessment`), which BMA-NFE has no table for.
...
2. The moment that transaction commits, the record is pushed to BMA-NFE.
   **Pressing Save is what sends it — there is no polling and no waiting for a
   scheduler.** By the time the partner's browser has the response, the record
   is either in BMA-NFE already or in the outbox with a retry scheduled.
3. `datasync.flush_sync_outbox` runs on Celery beat every five minutes. It is
   *only* a retry sweep for what step 2 could not deliver — it is not how
   changes normally travel.

The commit in step 2 matters: the push re-reads the record to build its
payload, so it must not run before that record is visible.

### Who performs the push

`DATASYNC_DELIVERY_MODE` decides. Every mode still pushes on save; they differ
only in who does the work.

| Mode | Behaviour | Trade-off |
| --- | --- | --- |
| `thread` *(default)* | The web process pushes it on a small background pool as the save commits. | Immediate, needs no Celery worker, and does not hold up the response. |
| `inline` | The web process pushes it before the response is returned. | The strictest reading of "on save" — but the partner waits for the round trip to BMA-NFE. |
| `celery` | A worker performs the push. | Only as immediate as the worker is, and needs one consuming the `datasync` queue. Falls back to the background pool if the broker is unreachable. |

`thread` is the default because it is genuinely save-time without adding the
network round trip to the partner's save. Pick `inline` only if you need the
save to fail visibly in step with replication; pick `celery` if you would
rather the web processes never make outbound calls.

A replication failure never breaks the save in any mode: the record is
committed, the event stays in the outbox, and the sweep retries it.
...
In the default `thread` mode, Celery is **not** on the path of a normal save —
the web process does the push itself. Celery is still needed for the retry
sweep, so `beater` must be running, and a worker must consume the `datasync`
queue. The `worker` process does that alongside `default`; split it out if you
would rather isolate it:

```
celery -A student_registration.taskapp.celery worker -Q datasync --loglevel=info
```

In `celery` mode a worker on that queue is also what performs every save-time
push, so it must be running for replication to be timely at all.
...
| `DATASYNC_DELIVERY_MODE` | `thread` | Who performs the save-time push (see above) |
| `DATASYNC_MAX_WORKERS` | `4` | Threads for save-time pushes in `thread` mode |
...
* `sex` is a 50-character column here and a 6-character one there; anything
  other than `Male`/`Female` is sent as empty rather than failing the push.
...
* `DigitalService.madrasti`, `LegoService.participating_lego_events`

Some choice lists are longer here than in BMA-NFE — `Summer RS Grade N` for
attendance and education programmes, `SDC` as a centre type, the "Internally
Displaced …" education statuses. They are stored there as sent (Postgres does
not enforce choices) but will not show in BMA-NFE's dropdowns until its
choice lists are extended.
