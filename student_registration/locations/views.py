...
from .forms import CenterForm, ProgramStaffForm
from .models import Center, Location, ProgramStaff
from .serializers import LocationSerializer, ProgramStaffSerializer
from .tables import BootstrapTable, CenterTable
from .utils import *
...
class ProfileView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        program_staffs = ProgramStaff.objects.filter(center__id=center_id).order_by(
            "facilitator_name"
        )
...
class CenterFormView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class CenterListView(
    ...
    group_required = ["MSCC", "MSCC_PARTNER"]
...
class ProgramStaffFormView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    def get_form(self, form_class=None):
        ...
        if self.request.method == "POST":
            return ProgramStaffForm(
                self.request.POST, pk=pk, center_id=center_id, request=self.request
            )
        else:
            if pk:
                instance = ProgramStaff.objects.get(id=pk)
                return ProgramStaffForm(
                    instance=instance, pk=pk, center_id=center_id, request=self.request
                )
...
def export_center_background(request):
    try:
        ...
        ExportHistory.objects.create(
            export_type="Center List", created_by=user, partner_name=partner_name
        )
