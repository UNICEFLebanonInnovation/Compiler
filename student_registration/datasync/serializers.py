...
#: ``Person.sex`` is a 50-character column here but a 6-character one in
#: BMA-NFE, so anything but the two real values would fail at its database.
TEACHER_SEX_VALUES = {'male': 'Male', 'female': 'Female'}


def _teacher_sex(value):
    """Return ``Male``/``Female``, or ``None`` for anything else."""
    if not value:
        return None
    return TEACHER_SEX_VALUES.get(str(value).strip().lower())
...
def serialize_package(instance):
    """Serialize a package definition.

    Packages are the templates the per-child services checklist is generated
    from. BMA-NFE matches them on name and type.
    """
    return {'fields': plain_fields(instance)}
...
#: Which model owns the ``service_id`` a checklist row points at, per resource.
_SERVICE_MODELS = {
    RESOURCE_PSS_SERVICE: PSSService,
    RESOURCE_INCLUSION_SERVICE: InclusionService,
    RESOURCE_DIGITAL_SERVICE: DigitalService,
    RESOURCE_HEALTH_NUTRITION_SERVICE: HealthNutritionService,
    RESOURCE_HEALTH_NUTRITION_REFERRAL: HealthNutritionReferral,
    RESOURCE_YOUTH_KIT_SERVICE: YouthKitService,
    RESOURCE_FOLLOW_UP_SERVICE: FollowUpService,
    RESOURCE_LEGO_SERVICE: LegoService,
    RESOURCE_EDUCATION_RS_SERVICE: EducationRSService,
}


def _service_reference(row):
    """Return ``{"resource", "source_id"}`` for a checklist row's ``service_id``.

    ``ProvidedServices.service_id`` is the local primary key of whichever
    service record completed that checklist item -- meaningless in the other
    database. The row's name says which model it belongs to; where two models
    share a name ("Health and Nutrition") the one that actually owns the id
    for this registration wins.

    The two tables sharing a name have independent id sequences, so both can
    own the same id for the same registration. ``update_service`` runs right
    after the record's own save, so the more recently modified owner is the
    one the row was last pointed at.

    Returns:
        dict | None: The reference, or ``None`` when the row has no service
        yet or the id no longer points at a live record.
    """
    if not row.service_id or not row.registration_id:
        return None

    owners = []
    for resource in SERVICE_NAME_RESOURCES.get(row.name, ()):
        model = _SERVICE_MODELS.get(resource)
        if model is None:
            continue
        instance = model.objects.filter(
            pk=row.service_id, registration_id=row.registration_id
        ).first()
        if instance is not None:
            modified = getattr(instance, 'modified', None)
            owners.append((modified.timestamp() if modified else 0.0, resource))

    if not owners:
        return None
    owners.sort(reverse=True)
    return {'resource': owners[0][1], 'source_id': row.service_id}


def _provided_services(instance):
    """Serialize the registration's services checklist."""
    rows = []
    for row in ProvidedServices.objects.filter(registration=instance).order_by('id'):
        rows.append({
            'source_id': row.pk,
            'fields': plain_fields(row),
            'service': _service_reference(row),
        })
    return rows


def _education_history(instance):
    """Serialize the registration's education history lines.

    ``child`` and ``registration_id`` are bare integer columns holding local
    primary keys, so they are sent as natural keys instead and BMA-NFE fills
    the columns from its own ids. ``programme_id`` points at CLM tables that
    only exist in the Compiler and is passed through as-is.
    """
    lines = []
    for line in EducationHistory.objects.filter(registration_id=instance.pk).order_by('id'):
        lines.append({
            'source_id': line.pk,
            'fields': plain_fields(line, exclude=('child', 'registration_id')),
        })
    return lines
...
def serialize_registration(instance):
    ...
    The services checklist and the education history travel with the
    registration because BMA-NFE never generates them itself -- it only reads
    them to draw the child profile -- and because the Compiler rewrites both
    wholesale (``regenerate_services``), which replace-the-set semantics
    mirror exactly.
    """
    return {
        'fields': plain_fields(instance),
        ...
        'partner': key_partner(instance.partner),
        'provided_services': _provided_services(instance),
        'education_history': _education_history(instance),
    }
...
def serialize_education_rs_service(instance):
    """Serialize a retention-support education service."""
    return {
        'fields': plain_fields(instance),
        'registration': key_registration(instance.registration),
        'school': key_school(instance.school),
    }


def serialize_registration_child(instance):
    """Serialize any record whose only relation is its registration.

    Covers every MSCC service and assessment table that is field-for-field
...
def serialize_grading(instance):
    ...
    return serialize_registration_child(instance)
...
REGISTRY = (
    ...
    (Packages, RESOURCE_PACKAGE, serialize_package),
    ...
    (EducationRSService, RESOURCE_EDUCATION_RS_SERVICE, serialize_education_rs_service),
    (EducationAssessment, RESOURCE_EDUCATION_ASSESSMENT, serialize_registration_child),
    (EducationProgrammeAssessment, RESOURCE_GRADING, serialize_grading),
    (Referral, RESOURCE_REFERRAL, serialize_referral),
    (PSSService, RESOURCE_PSS_SERVICE, serialize_registration_child),
    (InclusionService, RESOURCE_INCLUSION_SERVICE, serialize_registration_child),
    (DigitalService, RESOURCE_DIGITAL_SERVICE, serialize_registration_child),
    (HealthNutritionService, RESOURCE_HEALTH_NUTRITION_SERVICE, serialize_registration_child),
    (HealthNutritionReferral, RESOURCE_HEALTH_NUTRITION_REFERRAL, serialize_registration_child),
    (YouthKitService, RESOURCE_YOUTH_KIT_SERVICE, serialize_registration_child),
    (YouthService, RESOURCE_YOUTH_SERVICE, serialize_registration_child),
    (YouthAssessment, RESOURCE_YOUTH_ASSESSMENT, serialize_registration_child),
    (YouthReferral, RESOURCE_YOUTH_REFERRAL, serialize_registration_child),
    (FollowUpService, RESOURCE_FOLLOW_UP_SERVICE, serialize_registration_child),
    (Recreational, RESOURCE_RECREATIONAL, serialize_registration_child),
    (LegoService, RESOURCE_LEGO_SERVICE, serialize_registration_child),
    (MSCCAttendance, RESOURCE_ATTENDANCE, serialize_attendance),
)

#: Models whose save also rewrites the registration's services checklist
#: (through ``update_service``), and so must republish the registration too.
CHECKLIST_MODELS = tuple(_SERVICE_MODELS.values())
