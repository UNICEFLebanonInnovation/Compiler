...
class Student2ndShiftManager(models.Manager):
    def get_queryset(self):
        return (
            super(Student2ndShiftManager, self)
            .get_queryset()
            .filter(
...
class Person(TimeStampedModel):
    ...
    phone = models.CharField(
        max_length=64, blank=True, null=True, verbose_name=_("Phone number")
    )
    phone_prefix = models.CharField(
        max_length=10, blank=True, null=True, verbose_name=_("Phone prefix")
    )
    ...
    recordnumber = models.CharField(
        max_length=45, blank=True, null=True, verbose_name=_("Identity record number")
    )
...
class Student(Person):
    ...
    unhcr_family = models.CharField(
        max_length=150, blank=True, null=True, verbose_name=_("UNHCR Family Number")
    )
...
class Teacher(Person):
    ...
    teaching_hours_dirasa = models.IntegerField(
        blank=True, null=True, verbose_name=_("Number of teaching hours in Dirasa")
    )
    trainings = models.ManyToManyField(
        Training, blank=True, verbose_name=_("Topics of teacher training")
    )
