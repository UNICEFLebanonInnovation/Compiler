...
from student_registration.schools.models import ClassRoom, EducationLevel, EducationYear, School, Section
...
class Attendance(TimeStampedModel):
    ...
    absence_reason = models.CharField(
        max_length=50, blank=True, null=True, choices=ABSENCE_REASON
    )
    close_reason = models.CharField(
        max_length=50, blank=True, null=True, choices=CLOSE_REASON
    )
    ...
    def save(self, **kwargs):
        ...
        if self.students:
            ...
            for level_section in self.students:
                ...
                self.total_attended_male += self.students[level_section][
                    "total_attended_male"
                ]
                self.total_attended_female += self.students[level_section][
                    "total_attended_female"
                ]
                self.total_absent_male += self.students[level_section][
                    "total_absent_male"
                ]
                self.total_absent_female += self.students[level_section][
                    "total_absent_female"
                ]
...
class CLMAttendance(TimeStampedModel):
    ...
    attendance_date = models.DateField(
        blank=True, null=True, verbose_name=_("Attendance date")
    )
...
class CLMStudentAbsences(TimeStampedModel):
    ...
    def update_absence_statisics(
        self, consecutive_absences, ending_date, consecutive_dates
    ):
