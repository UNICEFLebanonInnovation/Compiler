...
from django.db.models import Avg, F, Func, OuterRef, Q, Subquery, Sum, When
...
from student_registration.schools.models import ClassRoom, CLMRound, EducationLevel, School, Section
from student_registration.users.utils import force_default_language

from .forms import AttendanceAbsenceForm, AttendanceStudentForm, MainAttendanceForm
# calculate_absentees
from .models import (
    Absentee,
    Attendance,
...
from .serializers import AbsenteeSerializer, AttendanceExportSerializer, AttendanceSerializer
from .utils import fill_attendancedt, find_attendances
...
class AttendanceViewSet(
    ...
    def list(self, request, *args, **kwargs):
        if self.request.GET.get("from_date", None) and self.request.GET.get(
            "to_date", None
        ):
            data = find_attendances(
                governorate=self.request.GET.get("governorate", None),
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            instance = Attendance.objects.get(
                school=int(request.POST.get("school")),
            ...
            return JsonResponse(
                {"status": status.HTTP_201_CREATED, "data": serializer.instance.id}
            )
...
class AbsenteeViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    model = Absentee
    ...
    def get_queryset(self):
        if self.request.GET.get("days", None):
            return self.queryset.filter(
                absent_days__lte=self.request.GET.get("days", None), absent_days__gte=5
            )
...
class AttendanceView(LoginRequiredMixin, GroupRequiredMixin, ListView):
    model = Attendance
    template_name = "attendances/school_day.html"
    group_required = ["ATTENDANCE"]
    ...
    def get_context_data(self, **kwargs):
        ...
        if not school.academic_year_start:
            messages.warning(
                self.request,
                _(
                    "Please go to the school profile and enter the academic start date "
        ...
        selected_date_view = datetime.datetime.strptime(
            selected_date, date_format
        ).strftime(date_format_display)
        ...
        queryset = Enrollment.objects.exclude(
            last_moved_date__lt=selected_date, moved=True
        ).filter(
            school_id=school, education_year__current_year=True, dropout_status=False
        )
        # queryset = Enrollment.objects.exclude(moved=True).filter(school_id=school, education_year=education_year)
        registrations = (
            queryset.filter(classroom__isnull=False, section__isnull=False)
            .distinct()
            .values("classroom__name", "classroom_id", "section__name", "section_id")
        ...
        for registry in registrations:
            ...
            level_section = "{}-{}".format(
                registry["classroom_id"], registry["section_id"]
            )
            attendances = (
                attendance.students[level_section]
                if attendance
                and attendance.students
                and level_section in attendance.students
                else ""
            )
            ...
            if attendances:
                ...
                exam_day = (
                    attendances["exam_day"] if "exam_day" in attendances else False
                )
                not_attending = (
                    attendances["not_attending"]
                    if "not_attending" in attendances
                    else False
                )
            ...
            if (
                level
                and section
                and level.id == registry["classroom_id"]
                ...
                if (
                    exam_day
                    or not_attending
                    or (attendance and attendance.validation_date)
                    or school_closed
                ):
        ...
        if level and section:
            ...
            for line in students:
                ...
                if str(student.id) in attendance_status:
                    ...
                    line.attendance_status = (
                        student_status["status"] if "status" in student_status else True
                    )
                    line.absence_reason = (
                        student_status["absence_reason"]
                        if "absence_reason" in student_status
                        else ""
                    )
        ...
        if school.attendance_from_beginning:
            ...
            day_range = (
                school.attendance_range
                if school.attendance_range
                else Attendance.DEFAULT_ATTENDANCE_RANGE
            )
        ...
        if selected_date not in allowed_dates:
            messages.warning(
                self.request,
                _(
                    "This dates is blocked you are not allowed to take attendance for this date."
                ),
            )
            self.template_name = "error.html"
            return {}
...
class AbsenteeView(ListAPIView):
    ...
    queryset = Absentee.objects.filter(school__location=True)
    serializer_class = AbsenteeSerializer
    permission_classes = (permissions.IsAdminUser,)


class AttendanceALPView(LoginRequiredMixin, GroupRequiredMixin, ListView):
    model = Attendance
    template_name = "attendances/school_day.html"
    group_required = ["ATTENDANCE"]
    ...
    def get_context_data(self, **kwargs):
        ...
        if not school.academic_year_start:
            messages.warning(
                self.request,
                _(
                    "Please go to the school profile and enter the academic start date in order to take attendance."
                ),
            )
            self.template_name = "error.html"
            return {}
        ...
        queryset = Outreach.objects.filter(
            school_id=school, alp_round__current_round=True
        )
        registrations = (
            queryset.filter(registered_in_level__isnull=False, section__isnull=False)
            .distinct()
            .values(
        ...
        for registry in registrations:
            ...
            attendance_taken = False
            level_section = "{}-{}".format(
                registry["registered_in_level_id"], registry["section_id"]
            )
            attendances = (
                attendance.students[level_section]
                if attendance
            ...
            total = queryset.filter(
                registered_in_level_id=registry["registered_in_level_id"],
                section_id=registry["section_id"],
            ).count()
            ...
            if (
                level
                and section
                and level.id == registry["registered_in_level_id"]
                and section.id == registry["section_id"]
            ):
                current_level_section = level_by_section
                if (
        ...
        if level and section:
            students = queryset.filter(
                registered_in_level_id=level.id,
                section_id=section.id,
            ).order_by(
                "student__first_name", "student__father_name", "student__last_name"
            )
...
class AttendancesExportViewSet(
    mixins.ListModelMixin, viewsets.GenericViewSet, SuperuserRequiredMixin
):
    """
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(
            attendance_date__day__gte=from_day, attendance_date__day__lte=to_day
        )
...
class ExportView(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        data = export_attendance(
            {"date": selected_date, "school": school, "school_type": school_type},
            return_data=True,
        )
        ...
        response["Content-Disposition"] = (
            "attachment; filename=attendance_" + selected_date + ".xlsx"
        )
        return response


class MainAttendanceCreateView(
    LoginRequiredMixin, GroupRequiredMixin, CreateView, RequestConfig
):
    ...
    def get_form_kwargs(self):
        ...
        if self.request.method == "POST":
            kwargs["attendance_student_formset"] = self.get_student_formset(
                self.request.POST
            )
        else:
            ...
            if school is not None:
                ...
                if school > 0 and registration_level != "" and day_off == "no":
                    ...
                    if attendance_date is not None:
                        queryset = queryset.filter(
                            Q(registration_date__isnull=True)
                            | Q(registration_date__lte=attendance_date),
                            Q(dropout_date__isnull=True)
            ...
            for line in queryset:
                ...
                data.append(student)
            kwargs["attendance_student_formset"] = self.get_initial_student_formset(
                data
            )
        ...
        kwargs["clm_bridging_all"] = self.request.user.groups.filter(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if attendance:
            ...
            return redirect(
                reverse(
                    "attendances:main_attendance_edit", kwargs={"pk": attendance_id}
                )
            )
        else:
    ...
    def form_valid(self, form, attendance_student_formset):
        ...
        for attendance_student in attendance_students:
            ...
            registration = Bridging.objects.filter(
    ...
    def form_invalid(self, form, attendance_student_formset):
        return self.render_to_response(
            self.get_context_data(
                form=form, attendance_student_formset=attendance_student_formset
            )
        )
    ...
    def get_initial(self):
        ...
        if self.request.GET.get("school", None):
            ...
            if self.request.GET.get("registration_level", None):
                initial_values["registration_level"] = self.request.GET.get(
                    "registration_level", ""
                )
            ...
            if self.request.GET.get("attendance_date", None):
                initial_values["attendance_date"] = self.request.GET.get(
                    "attendance_date", ""
                )
...
class MainAttendanceUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.helper.form_action = reverse(
            "attendances:main_attendance_edit", args=[attendance_id]
        )
...
def update_student_consecutive_absences(student_id, current_round, registration):
    ...
    student_absences = (
        CLMAttendanceStudent.objects.filter(
            attended="no",
            attendance_day__attendance_date__range=(
                current_round.start_date_bridging,
                current_round.end_date_bridging,
            ),
            student_id=student_id,
        )
        .order_by("student__id", "attendance_day__attendance_date")
        .all()
    )
    ...
    if student_absences:
        ...
        working_day_names = (
            ...
            .values_list("working_days", flat=True)
            .first()
        )

        current_student_absence = StudentConsecutiveAbsenceTracking()

        current_student_absence.initialise(
            student_id,
            partner_id,
            school_id,
            current_round,
        ...
        CLMStudentAbsences.objects.filter(
            student_id=student_id,
            round_id=current_round.id,
            absence_starting_date__range=(
                current_round.start_date_bridging,
                current_round.end_date_bridging,
            ),
        ).exclude(
            absence_starting_date__in=current_student_absence.absence_dates
        ).delete()
...
class StudentConsecutiveAbsenceTracking:
    ...
    def check_add_absence_record(current_student_absence):
        if current_student_absence.absence_starting_date is not None:
            ...
            current_student_absence.absence_dates.append(
                current_student_absence.absence_starting_date
            )
    ...
    def add_absence_record(current_student_absence):
        ...
        absence_exists = CLMStudentAbsences.objects.filter(
            student_id=student_id,
            round_id=round_id,
        ...
        if absence_exists:
            absence = CLMStudentAbsences.objects.filter(
                student_id=student_id,
                round_id=round_id,
                absence_starting_date=absence_starting_date,
            ).first()
        ...
        while start_date <= end_date:
            if not current_student_absence.is_date_off_weekend(
                start_date,
            ):
                current_student_absence.consecutive_dates.append(str(start_date))
            start_date += delta

        absence.update_absence_statisics(
            current_student_absence.absence_days,
            current_student_absence.absence_ending_date,
            current_student_absence.consecutive_dates,
        )
        absence.save()
    ...
    def check_update_absence_date(current_student_absence, absence_date):
        ...
        if current_student_absence.absence_starting_date is None:
        ...
        elif current_student_absence.all_range_off(
            current_student_absence.absence_ending_date, absence_date
        ):
            ...
            current_student_absence.absence_ending_date + datetime.timedelta(days=1)
        ):
    ...
    def all_range_off(current_student_absence, previous_absence_date, new_absence_date):
        ...
        while current_absence_date != new_absence_date and result:
            ...
            result = result and current_student_absence.is_date_off_weekend(
                current_absence_date,
            )
...
def update_student_total_attandance(student_id, current_round, registration):
    ...
    absence_exists = CLMStudentTotalAttendance.objects.filter(
        student_id=student_id, registration_id=registration.id
    ).exists()

    if absence_exists:
        absence = CLMStudentTotalAttendance.objects.filter(
            student_id=student_id, registration_id=registration.id
        ).first()
    else:

        absence = CLMStudentTotalAttendance(
            round_id=round_id,
            student_id=student_id,
            partner_id=registration.partner.id,
...
def absence_export(request, number_of_absences, total_days):
    ...
    consecutive_student_id = CLMStudentAbsences.objects.filter(
        consecutive_absence_days__gte=number_of_consecutive_absences, round_id=round_id
    ).values_list("student_id", flat=True)
    total_student_id = CLMStudentTotalAttendance.objects.filter(
        total_absence_days__gte=number_of_total_absence, round_id=round_id
    ).values_list("student_id", flat=True)

    consecutive_student_list = list(consecutive_student_id)
    total_student_list = list(total_student_id)

    student_ids = consecutive_student_list + list(
        set(total_student_list) - set(consecutive_student_list)
    )
    student_ids = list(set(student_ids))

    consecutive_absent_students = (
        CLMStudentAbsences.objects.filter(student_id__in=student_ids)
    ...
    for row in consecutive_absent_students:
        ...
        total_absent_students = CLMStudentTotalAttendance.objects.filter(
            student_id=student_id, round_id=round_id
        ).last()

        ws.write(
            row_num_student, 9, total_absent_students.total_attendance_days, font_style
        )
        ws.write(
            row_num_student, 10, total_absent_students.total_absence_days, font_style
        )
...
def consecutive_absence_export(request, **kwargs):
    try:
        ...
        query_params = [round_id]
        vw_data_str = (
            "SELECT * FROM vw_bridging_absence_consecutive WHERE round_id = %s"
        )
