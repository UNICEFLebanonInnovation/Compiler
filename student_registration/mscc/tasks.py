...
def generate_mscc_export(export_id, fields=None, file_format='csv'):
    ...
    try:
        ...
        # Generate a time-limited SAS URL so the blob can be downloaded from
        # private Azure storage without exposing the container publicly. The
        # default link generated by ``storage.url`` does not include a token
        # when ``AZURE_URL_EXPIRATION_SECS`` is unset which results in a
        # ``ResourceNotFound`` error when accessed by the user. Supplying an
        # explicit expiry ensures a signed URL is returned.
        file_url = storage.url(file_name, expire=60 * 60)
...
def generate_filtered_mscc_export(export_id, nationality="", first_name="", last_name="",
    ...
    try:
        ...
        storage = ExportStorage()
        storage.save(file_name, ContentFile(zip_output.getvalue()))
        # Return a signed URL so the blob can be accessed even when the Azure
        # container is private. Without an explicit expiry a bare URL is
        # returned which causes a 404 "ResourceNotFound" error when users try
