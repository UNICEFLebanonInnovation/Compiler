...
from .forms import MainForm, ReferralForm
from .models import EducationHistory, MSCCExportRequest, Referral, Registration
from .serializers import MainSerializer
from .tables import BootstrapTable, FullTable, MainTable, PartnerTable, YouthMainTable
...
class DashboardCustomView(LoginRequiredMixin, TemplateView):
    template_name = "mscc/dashboard.html"
...
class DashboardYouthView(LoginRequiredMixin, TemplateView):
...
class MainAddView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    form_class = MainForm
    group_required = ["MSCC", "MSCC_CENTER"]

    def get_success_url(self):
        return reverse(
            "mscc:child_profile", kwargs={"pk": self.request.session.get("instance_id")}
        )
    ...
    def get_form(self, form_class=None):
        ...
        if self.request.method == "POST":
            ...
            data["child_nationality"] = (
                data["child_nationality_id"] if "child_nationality_id" in data else ""
            )
            data["child_disability"] = (
                data["child_disability_id"] if "child_disability_id" in data else ""
            )
...
class NewRoundView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
...
class NewRoundRedirectView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self):
        ...
        if self.request.GET.get("new_round_confirmation", None) == "confirmed":
            ...
            generate_services(
                new_registration.child.age, new_registration, self.request.user
            )
            return reverse(
                "mscc:service_education_add",
                kwargs={"registry": new_registration.id, "package_type": type},
            )
...
def main_registration_cancel_view(request, pk):
    if request.user.is_authenticated:
        try:
            ...
            registration.deleted = True
            registration.save()
            return redirect(reverse("mscc:list"))
        except Registration.DoesNotExist:
            result = {"isSuccessful": False}
    else:
        result = {"isSuccessful": False}
...
class ReferralFormView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    def get_success_url(self):
        return (
            reverse("mscc:child_profile", kwargs={"pk": self.kwargs["registry"]})
    ...
    def get_form(self, form_class=None):
        ...
        if self.request.method == "POST":
            return ReferralForm(
                self.request.POST, pk=pk, registry=registry, request=self.request
            )
        else:
            if pk:
                ...
                return ReferralForm(
                    instance=instance, registry=registry, pk=pk, request=self.request
                )
        ...
        if user.is_authenticated:
            ...
            if has_group(user, "MSCC_UNICEF"):
                qs = Registration.objects.filter(
                    Q(round__isnull=True) | Q(round__current_year=True), deleted=False
                ).order_by("-id")
            elif has_group(user, "MSCC_PARTNER") and partner_id:
                qs = Registration.objects.filter(
                    Q(round__isnull=True) | Q(round__current_year=True),
...
class ProgrammeDetails(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return {"instance": instance, "programme_type": programme_type}
...
def export_list_background(request):
    try:
        ...
        if not round:
            return JsonResponse(
                {
                    "error": "Round is not selected. Please select a round before exporting data."
                },
                status=400,
            )
        ...
        ExportHistory.objects.create(
            export_type="Makani List", created_by=user, partner_name=partner_name
        )
        ...
        try:
            with storage.open(file_path, "rb") as f:
                ...
                response["Content-Disposition"] = (
                    'attachment; filename="' + returned_file_name + '"'
                )
        except Exception as e:
