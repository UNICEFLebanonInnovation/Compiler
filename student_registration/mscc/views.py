...
class EducationSituationView(LoginRequiredMixin,
    ...
    template_name = 'mscc/education_situation.html'
    form_class = EducationSituationForm
    success_url = '/mscc/list/'
    group_required = [u"MSCC"]
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'form' not in kwargs:
            kwargs['form'] = self.get_form()
        return super(EducationSituationView, self).get_context_data(**kwargs)
    ...
    def form_valid(self, form):
        instance = Registration.objects.get(id=self.kwargs['pk'], partner=self.request.user.partner_id)
        form.save(request=self.request, instance=instance)
        return super(EducationSituationView, self).form_valid(form)


class DiagnosticAssessmentView(LoginRequiredMixin,
                             GroupRequiredMixin,
                             FormView):
    template_name = 'mscc/diagnostic_assessment.html'
...
class EducationAssessmentView(LoginRequiredMixin,
                             GroupRequiredMixin,
                             FormView):
    template_name = 'mscc/education_assessment.html'
