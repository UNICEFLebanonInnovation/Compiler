/**
 * Created by Ali on 2016-08-26.
 */

setInterval(function () { window.applicationCache.update(); }, 3600); // Check for an updated manifest file every 60 minutes. If it's updated, download a new cache as defined by the new manifest file.

window.applicationCache.addEventListener('updateready', function(){ // when an updated cache is downloaded and ready to be used
        console.log('update page');
        window.applicationCache.swapCache(); //swap to the newest version of the cache
}, false);
