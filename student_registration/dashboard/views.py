...
from student_registration.schools.models import ClassLevel, ClassRoom, EducationLevel, EducationYear, Evaluation, School
from student_registration.users.models import User
from student_registration.users.utils import force_default_language


class ExporterView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
...
class RunExporterViewSet(LoginRequiredMixin, GroupRequiredMixin, ListView):
...
def dup_id_enr(request, *args):
    ...
    Q_Enr = Enrollment.objects.filter(
        education_year_id=education_year,
        school_id__isnull=False,
        school__is_2nd_shift=True,
        disabled=False,
        moved=False,
    )
    for q_enr in Q_Enr:
        Q_If_Exist_Enr = Enrollment.objects.filter(
            education_year_id=education_year,
            student_id=q_enr.student_id,
            school_id__isnull=False,
            moved=False,
            disabled=False,
            school__is_2nd_shift=True,
        ...
        for q_if_exist_enr in Q_If_Exist_Enr:
            Q_duplicatestd = DuplicateStd.objects.filter(
                enrollment_id=q_if_exist_enr.id, is_solved=False
            ).count()
            if Q_duplicatestd == 0:
...
def update_duplicatestd(request):
    ...
    education_year = EducationYear.objects.get(current_year=True)
    Q_duplicatestd = DuplicateStd.objects.filter(
        is_solved=False, education_year=education_year
    )
    for q_duplicatestd in Q_duplicatestd:
        Q_student = Student.objects.filter(
            number=q_duplicatestd.enrollment.student.number
        )
        str = []
        for q_student in Q_student:
            str.append(q_student.id)

        v_exist = Enrollment.objects.filter(
            education_year_id=education_year,
            school_id__isnull=False,
            school__is_2nd_shift=True,
            student_id__in=str,
...
def fill_data(request, *args):
    ...
    T_Enrollment = Enrollment.objects.filter(
        created__gt="2019-10-9",
        created__lt="2019-10-11",
        school_id__isnull=True,
        education_year_id__isnull=True,
    )  # classroom_id__isnull=True,
    for T_Enr in T_Enrollment:
        try:
            last_enrollment = Enrollment.objects.filter(
                student_id=T_Enr.student_id, education_year_id=1
            ).order_by("-created")[:1]
...
def run_attendance(request):
    from student_registration.attendances.tasks import (
        geo_calculate_attendances_by_student,
...
def run_to_excel_per_day(request):
    ...
    if request.GET.get("btntype") == "Generate":
        geo_calculate_attendances_per_day(
            from_school, to_school, from_date, to_date, txt_std
        )
        messages.add_message(request, messages.INFO, "Finished !")
        context = {}
        return render(request, "dashboard/exporter.html", context)
        ...
        if txt_std:
            ...
            for st in std:
                std_id = (st.id,)
            qs_attendancedt = AttendanceDt.objects.filter(
                attendance_date__gte=from_date,
                attendance_date__lte=to_date,
                school__number__gte=from_school,
                school__number__lte=to_school,
...
class RegistrationsALPView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    """
...
class RegistrationsALPOverallView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    """
    Provides the registration page with lookup types in the context
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        alp_rounds = ALPRound.objects.all()
        enrolled = self.queryset.filter(
            registered_in_level__isnull=False, alp_round=alp_round
        )
        ...
        not_schools = User.objects.filter(
            groups__name__in=["PARTNER", "TEST_MANAGER", "CERD"]
        )
        ...
        return {
            ...
            "referred_to_formal_males": referred_to_formal.filter(
                student__sex="Male"
            ).count(),
            "referred_to_formal_females": referred_to_formal.filter(
                student__sex="Female"
            ).count(),
            "referred_to_following": referred_to_following.count(),
            "referred_to_following_males": referred_to_following.filter(
                student__sex="Male"
            ).count(),
            "referred_to_following_females": referred_to_following.filter(
                student__sex="Female"
            ).count(),
            "repeated_level": repeated_level.count(),
            "repeated_level_males": repeated_level.filter(student__sex="Male").count(),
            "repeated_level_females": repeated_level.filter(
                student__sex="Female"
            ).count(),
            ...
            "new_enrolled_test_males": new_enrolled_test.filter(
                student__sex="Male"
            ).count(),
            "new_enrolled_test_females": new_enrolled_test.filter(
                student__sex="Female"
            ).count(),
            "alp_round": alp_round,
...
class RegistrationsALPOutreachView(
...
class RegistrationsALPPreTestView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    """
    Provides the registration page with lookup types in the context
    """
...
class RegistrationsALPPostTestView(
    LoginRequiredMixin, GroupRequiredMixin, TemplateView
):
    """
    Provides the registration page with lookup types in the context
...
def generate_pretest_result(request):
    ...
    for registry in registrations:
        try:
            registry.assigned_to_level = assign_to_level(
                registry.level, registry.exam_total
            )
            registry.save()
    ...
    def get_context_data(self, **kwargs):
        ...
        if v_exist:
            v_last_nb = (
                ...
                .values_list("justificationnumber", flat=True)
                .order_by("-justificationnumber")[0]
            )
        else:
            v_last_nb = ""
        return {
    ...
    def get_context_data(self, **kwargs):
        ...
        return {
            "enrollment": Enrollment.objects.filter(
                ...
                school_id__isnull=False,
                school__is_2nd_shift=True,
                disabled=False,  # dropout_status=False, moved=False,
                school_id=self.request.user.school_id,
            ).order_by("classroom_id", "section_id", "student"),
            "education_year": education_year,
            "current_date": datetime.now(),
        }
...
def generate_justification_number(request):
    ...
    Q_Enr = (
        Enrollment.objects.filter(
            ...
            justificationnumber__isnull=True,
    ...
    q_count = (
        ...
        .exclude(justificationnumber="")
        .distinct("justificationnumber")
        .order_by("justificationnumber")
        .count(),
    )
    for q_enr in Q_Enr:
        q_enr.update(
            justificationnumber=str(request.user.school.number)
            + "-"
            + str(education_year)
            + "-"
            + str(int(q_count[0]) + 1)
        ...
        if request.GET["txt_school"]:
            Q_Enr = (
                Enrollment.objects.filter(
                    ...
                    school_id__isnull=False,
                    school__is_2nd_shift=True,  # dropout_status=False,  moved=False,
                    school__number=request.GET["txt_school"],
                ).exclude(justificationnumber=""),
            )

            for q_enr in Q_Enr:
                q_enr.update(justificationnumber=None)
            Q_Users = User.objects.filter(
                school__number=request.GET["txt_school"], is_active=True
            )
            group = Group.objects.get(name="ENROL_CREATE")
            for q_users in Q_Users:
...
class AttendanceListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        list_data = []
        for enr in ENR:
            ATTEND = AttendanceDt.objects.filter(
                attendance_date__gte=from_date,
                attendance_date__lte=to_date,
                is_present=True,
            ...
            ABSENCES = AttendanceDt.objects.filter(
                attendance_date__gte=from_date,
                attendance_date__lte=to_date,
                is_present=False,
                student_id=enr.student_id,
            ...
            if (
                (
                    (self.request.GET["rb_option"] == "rb_nozero_attend")
                    and (ATTEND != 0)
                )
                or (self.request.GET["rb_option"] == "rb_all")
                or (
                    (self.request.GET["rb_option"] == "rb_zero_attend")
                    and (ATTEND == 0)
                )
            ):
                list_data.append(
                    {
                        "groupby": "{} - {} - {}".format(
                            enr.school, enr.classroom, enr.section
                        ),
    ...
    if (from_school) and (to_date):
        qs_enr = (
            ...
            .extra(
                ...
                select_params=[from_date, to_date, from_date, to_date],
            )
            .order_by(
                "school_id",
                "classroom_id",
...
def generate_evaluation(request):
    ...
    education_year = EducationYear.objects.get(current_year=True)
    for sch in school:
        count_evaluation = Evaluation.objects.filter(
            school_id=sch.id, education_year=education_year
        ).count()
