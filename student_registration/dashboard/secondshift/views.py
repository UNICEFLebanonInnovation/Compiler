...
from student_registration.schools.models import ClassRoom, EducationYear, School
from student_registration.students.models import Nationality


class GovernorateGradeView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    model = Enrollment
    queryset = (
        Enrollment.objects.exclude(moved__isnull=True)
...
class GovernorateAgeView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    model = Enrollment
    queryset = (
        Enrollment.objects.exclude(moved__isnull=True)
        .exclude(dropout_status__isnull=True)
...
class GovernorateNationalityView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    model = Enrollment
    queryset = (
        Enrollment.objects.exclude(moved__isnull=True)
        .exclude(dropout_status__isnull=True)
...
class GradeAgeView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    model = Enrollment
    queryset = (
        Enrollment.objects.exclude(moved__isnull=True)
        .exclude(dropout_status__isnull=True)
...
class GradeNationalityView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    model = Enrollment
    queryset = (
        Enrollment.objects.exclude(moved__isnull=True)
        .exclude(dropout_status__isnull=True)
...
class NationalityAgeView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    model = Enrollment
    queryset = (
        Enrollment.objects.exclude(moved__isnull=True)
        .exclude(dropout_status__isnull=True)
...
class SchoolGradeView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    model = Enrollment
    queryset = (
        Enrollment.objects.exclude(moved__isnull=True)
        .exclude(dropout_status__isnull=True)
    ...
    def get_context_data(self, **kwargs):
        ...
        if governorate:
            ...
            schools = School.objects.filter(
                location__parent_id=selected_gov.id, is_2nd_shift=True
            )
        ...
        if district:
            ...
            queryset = queryset.filter(school__location_id=selected_district.id)
            schools = School.objects.filter(
                location_id=selected_district.id, is_2nd_shift=True
            )
...
class SchoolNationalityView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
    model = Enrollment
    queryset = (
        Enrollment.objects.exclude(moved__isnull=True)
        .exclude(dropout_status__isnull=True)
