...
from .filters import FullFilter, MainFilter, PDFilter, PDPartnerFilter
from .forms import MainForm
from .models import EnrolledPrograms, ProgramDocument, ProgramDocumentIndicator, SubProgram
from .serializers import MainSerializer
from .tables import PDPartnerTable, PDTable, RegistrationTable
from .utils import *
...
class ProfileView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return {"instance": instance, "current_tab": current_tab}


class MainAddView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    template_name = "youth/main_form.html"
    form_class = MainForm
    success_url = "/youth/List/"
    group_required = ["YOUTH"]
...
class MainEditView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    template_name = "youth/main_form.html"
    form_class = MainForm
    success_url = "/youth/List/"
    group_required = ["YOUTH"]
    ...
    def get_form(self, form_class=None):
        ...
        if self.request.method == "POST":
            ...
            data["adolescent_nationality"] = (
                data["adolescent_nationality_id"]
                if "adolescent_nationality_id" in data
                else ""
            )
            data["adolescent_governorate"] = (
                data["adolescent_governorate_id"]
                if "adolescent_governorate_id" in data
                else ""
            )
            data["adolescent_district"] = (
                data["adolescent_district_id"]
                if "adolescent_district_id" in data
                else ""
            )
            data["adolescent_cadaster"] = (
                data["adolescent_cadaster_id"]
                if "adolescent_cadaster_id" in data
                else ""
            )
            data["adolescent_disability"] = (
                data["adolescent_disability_id"]
                if "adolescent_disability_id" in data
                else ""
            )
            data["main_caregiver_nationality"] = (
                data["main_caregiver_nationality_id"]
                if "main_caregiver_nationality_id" in data
                else ""
            )
            data["father_educational_level"] = (
                data["father_educational_level_id"]
                if "father_educational_level_id" in data
                else ""
            )
            data["mother_educational_level"] = (
                data["mother_educational_level_id"]
                if "mother_educational_level_id" in data
                else ""
            )
...
class NewRoundRedirectView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self):
        ...
        if self.request.GET.get("new_round_confirmation", None) == "confirmed":
            ...
            return reverse(
                "youth:service_enrolled_programs_add",
                kwargs={"registry": new_registration.id},
            )
...
class MainListView(
    ...
    def get_queryset(self):
        ...
        if has_group(user, "YOUTH_UNICEF"):
            queryset = Registration.objects.filter(deleted=False).order_by("-id")
...
def outreach_child_search(request):
    ...
    filtered_results = OutreachChild.objects.filter(birthday_year=birthday_year)

    if birthday_month:
        filtered_results = filtered_results.filter(birthday_month=birthday_month)
    if birthday_day:
        filtered_results = filtered_results.filter(birthday_day=birthday_day)
    ...
    for result in filtered_results:
        result_str = "{} {} {}".format(
            result["first_name"],
            result["outreach_caregiver__father_name"],
...
def old_child_search(request):
    ...
    filtered_results = Student.objects.filter(birthday_year=birthday_year)
    ...
    for result in filtered_results:
        result_str = "{} {} {}".format(
            result["first_name"], result["father_name"], result["last_name"]
        )
        fuzzy_match = fuzz.ratio(form_str, result_str)
        if fuzzy_match > 70:
            result["score"] = fuzzy_match
...
def child_duplication_check(request):
    ...
    for result in filtered_results:
        result_str = "{} {} {}".format(
            result["child__first_name"],
            result["child__father_name"],
...
def export_data(request, **kwargs):
    ...
    try:
        ...
        adolescent_first_phone_number = request.GET.get(
            "adolescent_first_phone_number", ""
        )
        ...
        if adolescent_first_name:
            queryset = queryset.filter(
                registration__adolescent__first_name=adolescent_first_name
            )

        if adolescent_father_name:
            queryset = queryset.filter(
                registration__adolescent__father_name=adolescent_father_name
            )

        if adolescent_last_name:
            queryset = queryset.filter(
                registration__adolescent__last_name=adolescent_last_name
            )

        if adolescent_number:
            queryset = queryset.filter(
                registration__adolescent__number=adolescent_number
            )

        if adolescent_gender:
            queryset = queryset.filter(
                registration__adolescent__gender=adolescent_gender
            )

        if adolescent_nationality:
            queryset = queryset.filter(
                registration__adolescent__nationality_id=adolescent_nationality
            )

        if adolescent_disability:
            queryset = queryset.filter(
                registration__adolescent__disability__id=adolescent_disability
            )

        if adolescent_first_phone_number:
            queryset = queryset.filter(
                registration__adolescent__first_phone_number=adolescent_first_phone_number
            )
        ...
        logger.debug(
            cursor.mogrify(vw_youth_data_str, query_params).decode("utf-8")
        ...
        response["Content-Disposition"] = (
            "attachment; filename=youth_registration_data.csv"
        )
...
def export_pd_data(request, **kwargs):
    ...
    try:
        ...
        if implementing_partners:
            queryset = queryset.filter(
                implementing_partners__icontains=implementing_partners
            )
        ...
        if master_program:
            master_program_ids = [
                int(mp) for mp in master_program.split(",") if mp.isdigit()
            ]
...
class PDListView(
    ...
    def get_queryset(self):
        ...
        if has_group(user, "YOUTH_UNICEF"):
            ...
            queryset = ProgramDocument.objects.filter(partner=partner_id).order_by(
                "-id"
            )
            queryset = queryset.distinct("id")
            return queryset
...
def load_districts(request):
    ...
    if request.GET.get("id_adolescent_governorate"):
        ...
        cities = Location.objects.filter(parent_id=id_adolescent_governorate).order_by(
            "name"
        )
...
def load_cadasters(request):
    ...
    if request.GET.get("id_adolescent_district"):
        ...
        cities = Location.objects.filter(parent_id=id_adolescent_district).order_by(
            "name"
        )
    return render(
        request, "youth/cadaster_dropdown_list_options.html", {"cities": cities}
    )
...
def load_program_document(request):
    ...
    if request.GET.get("id_donor"):
        ...
        program_documents = ProgramDocument.objects.filter(
            donors__id=id_donor
        ).order_by("project_name")
    return render(
        request,
        "youth/program_document_dropdown_list_options.html",
...
def load_master_program(request):
    ...
    if request.GET.get("id_program_document"):
        ...
        program_document = ProgramDocument.objects.filter(
            id=id_program_document
        ).first()

        if program_document:
            master_programs = MasterProgram.objects.filter(
                id__in=program_document.indicators.filter(
    ...
    return render(
        request,
        "youth/master_program_dropdown_list_options.html",
...
def load_sub_program(request):
    ...
    if request.GET.get("id_master_program"):
        ...
        sub_programs = SubProgram.objects.filter(
            master_program_id=id_master_program
        ).order_by("name")
    return render(
        request,
        "youth/sub_program_dropdown_list_options.html",
        {"sub_programs": sub_programs},
    )


def program_document_indicators_view(request, program_document_id):
    master_indicators = list(
        MasterProgram.objects.filter(active=True).values("id", "name")
    )
    sub_indicator_map = {
        mp.id: list(
            SubProgram.objects.filter(master_program=mp)
            .order_by("name")
            .values("id", "name")
        )
    ...
    return render(
        request,
        "youth/program_document_indicator.html",
        {
...
def program_document_indicator_list_view(request, program_document_id):
    try:
        indicators = ProgramDocumentIndicator.objects.filter(
            program_document_id=program_document_id
        )
        data = []
            ...
            data.append(
                {
                    "id": ind.id,
                    "master_indicator_id": (
                        ind.master_indicator.id if ind.master_indicator else None
                    ),
                    "master_indicator_name": (
                        ind.master_indicator.name if ind.master_indicator else ""
                    ),
                    "sub_indicator_id": (
                        ind.sub_indicator.id if ind.sub_indicator else None
                    ),
                    "sub_indicator_name": (
                        ind.sub_indicator.name if ind.sub_indicator else ""
                    ),
