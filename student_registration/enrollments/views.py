...
from .filters import EnrollmentByRegionFilter, EnrollmentFilter, EnrollmentOldDataFilter
from .forms import (
    EditOldDataForm,
    EnrollmentForm,
...
from .models import EducationYear, Enrollment, EnrollmentGrading, LoggingProgramMove, LoggingStudentMove
...
from .tables import BootstrapTable, EnrollmentBySchoolTable, EnrollmentOldDataTable, EnrollmentTable
...
class EditView(LoginRequiredMixin, GroupRequiredMixin, FormView):

    template_name = "bootstrap4/common_form.html"
    form_class = EnrollmentForm
    success_url = "/enrollments/list/"
    group_required = ["ENROL_EDIT"]
    ...
    def get_form(self, form_class=None):
        instance = Enrollment.objects.get(
            id=self.kwargs["pk"], school=self.request.user.school
        )
        if self.request.method == "POST":
...
class EditRegionView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    template_name = "bootstrap4/common_form.html"
...
class EditOldDataView(LoginRequiredMixin, GroupRequiredMixin, FormView):

    template_name = "bootstrap4/common_form.html"
    form_class = EditOldDataForm
    success_url = "/enrollments/list-old-data/"
    group_required = ["ENROL_EDIT_OLD"]
...
class ListingOldDataView(
    ...
    def get_queryset(self):
        ...
        return Enrollment.objects.exclude(moved=True).filter(
            education_year__id__lt=education_year.id, school=self.request.user.school_id
        )


class MovedView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    def get_form(self, form_class=None):
        instance = Enrollment.objects.get(id=self.kwargs["pk"])
        if self.request.method == "POST":
            return StudentMovedForm(
                self.request.POST, instance=instance, moved=self.kwargs["moved"]
            )
        else:
...
class GradingView(LoginRequiredMixin, GroupRequiredMixin, FormView):

    template_name = "enrollments/grading.html"
    form_class = GradingTermForm
    success_url = "/enrollments/list/"
    group_required = ["ENROL_GRADING"]
    ...
    def get_form(self, form_class=None):
        ...
        instance = EnrollmentGrading.objects.get(
            id=self.kwargs["pk"], enrollment__school=self.request.user.school
        )
        if self.request.method == "POST":
...
class LoggingProgramMoveViewSet(
    ...
    def post(self, request, *args, **kwargs):
        if request.POST.get("dropout_status", 0):
            enrollment = Enrollment.objects.get(
                id=request.POST.get("dropout_status", 0)
            )
...
class StudentJustifyViewSet(
    ...
    def post(self, request, *args, **kwargs):
        if request.POST.get("justify_status", 0):
            enrollment = Enrollment.objects.get(
                id=request.POST.get("justify_status", 0)
            )
...
class LoggingStudentMoveViewSet(
    mixins.RetrieveModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet
):
    ...
    def get_queryset(self):
        ...
        if terms:
            qs = self.queryset.filter(
                school_to__isnull=True, education_year=current_year
...
class EnrollmentViewSet(
    ...
    def get_queryset(self):
        ...
        if self.request.user.school_id:
            return self.queryset.filter(school_id=self.request.user.school_id).order_by(
    ...
    def delete(self, request, *args, **kwargs):
        instance = self.model.objects.get(
            id=kwargs["pk"], school=self.request.user.school
        )
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("year", 0):
            queryset = queryset.filter(
                education_year=int(self.request.GET.get("year", 0))
            )
...
class EnrollmentGradingImportViewSet(
    mixins.ListModelMixin, viewsets.GenericViewSet, SuperuserRequiredMixin
):
    """
    Provides API importation Enrollment records
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("year", 0):
            queryset = queryset.filter(
                enrollment__education_year=int(self.request.GET.get("year", 0))
            )
        if self.request.GET.get("offset", 0):
            offset = int(self.request.GET.get("offset", 0))
            limit = offset + max_raw
...
class ExportBySchoolView(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for school in schools:
            ...
            content = [school[1], school[2], nbr, school[5], school[3], school[4]]
            data.append(content)
    ...
    def get_form(self, form_class=None):
        ...
        if self.request.method == "POST":
            ...
            if self.request.POST.get("birth_documenttype"):
                instance.birth_documenttype_id = self.request.POST.get(
                    "birth_documenttype"
                )
            else:
...
def changing_birthdoc(request, *args):
    from student_registration.students.models import Birth_DocumentType, Student
    ...
    if request.method == "POST":
        ...
        if request.POST.get("cb_documenttype", False):
            std.birth_documenttype = Birth_DocumentType.objects.get(
                id=request.POST["cb_documenttype"]
            )
        std.save()
...
def empty_doclastyear(request, *args):
    ...
    enr.save()
    return HttpResponse(
        "Document of the last year picture has been successfully cleaned "
    )


def changing_doclastyear(request, *args):
    from student_registration.enrollments.models import DocumentType, Enrollment
    ...
    return HttpResponseForbidden(
        "Document of the last year picture has been successfully changed"
    )
...
def clear_pic(request, *args):
    ...
    if (
        request.POST.get("chk_profile", False)
        or request.POST.get("chk_document", False)
        or request.POST.get("chk_unhcr", False)
    ):
