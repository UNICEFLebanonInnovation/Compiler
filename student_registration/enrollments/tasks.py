...
def assign_section(section):
    ...
    registrations = Enrollment.objects.exclude(deleted=True).exclude(dropout_status=True).filter(section__isnull=True)
...
def generate_2ndshift_report(school=0, location=0, email=None, user=None):
    ...
    offset = 120000
    limit = 180000
    queryset = Enrollment.objects.exclude(deleted=True).exclude(dropout_status=True)[offset:limit]
