...
from .filters import BridgingFullFilter, BridgingPartnerFilter
from .tables import BridgingTable
from .models import BLN, ABLN, RS, CBECE, Assessment, Outreach, Bridging, Inclusion
            ...
            if "post_test" in data:
                ...
                if p_test:
                    for key in [
                        ...
                        if f"Bridging_ASSESSMENT/{key}" in p_test:
                            data[key] = p_test[f"Bridging_ASSESSMENT/{key}"]
...
class BridgingPreAssessmentView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    if terms:
        if len(terms.split()) > 1:
            ...
            qs = (
                ...
                .distinct()
                .annotate(search_model=Value(search_model, output_field=CharField()))
            )
            ...
            # .filter(partner=request.user.partner_id)
            qs = (
                model.objects.filter(deleted=False)
                .filter(
...
def search_kobo_outreach_child(request):
    ...
    if terms:
        if len(terms.split()) > 1:
            ...
            qs = (
                OutreachChild.objects.annotate(
                    fullname=Concat(
                        "first_name",
                        Value(" "),
    ...
    if body_unicode:
        ...
        if unicef_id:
            qs = Bridging.objects.filter(
                round_id=round_id, student__unicef_id=unicef_id, deleted=False
            )
            if registration_id:
...
def bridging_export_all(request, **kwargs):
    try:
        ...
        csv_output = io.StringIO()
        csv_output.write("\ufeff")  # UTF-8 BOM for Arabic Excel support
