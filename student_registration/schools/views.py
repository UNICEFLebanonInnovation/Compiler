...
from .serializers import ClassRoomSerializer, SchoolSerializer, SectionSerializer
from .tables import ClubTable, CommunityInitiativeTable, HealthVisitTable, MeetingTable, SchoolExportTable, SchoolTable
from .utils import *
...
class ProfileView(LoginRequiredMixin, GroupRequiredMixin, FormView):

    template_name = "schools/profile.html"
    form_class = ProfileForm
    success_url = "/schools/profile/"
    group_required = ["SCHOOL", "ALP_SCHOOL"]
    ...
    def get_context_data(self, **kwargs):
        ...
        if "form" not in kwargs:
            ...
            tickets = Notification.objects.filter(type="helpdesk", school_id=school.id)
...
class PartnerView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class PublicDocumentView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
...
class EvaluationView(FormView):
    ...
    def form_valid(self, form):
        education_year = EducationYear.objects.get(current_year=True)
        instance = Evaluation.objects.get(
            school_id=self.request.user.school_id, education_year=education_year
        )
        form.save(request=self.request, instance=instance)
...
class SchoolAddView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    def get_success_url(self):
        ...
        if self.request.POST.get("save_and_continue", None):
            return (
                "/clm/school-edit/" + str(self.request.session.get("instance_id")) + "/"
            )
        return self.success_url
...
class SchoolEditView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class ClubListView(
    ...
    def get_context_data(self, **kwargs):
        """Insert the form into the context dict."""
        kwargs["school_id"] = (
            self.kwargs["school_id"] if "school_id" in self.kwargs else None
        )
        return super(ClubListView, self).get_context_data(**kwargs)


class ClubFormView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    def get_form(self, form_class=None):
        ...
        if self.request.method == "POST":
            return ClubForm(
                self.request.POST, pk=pk, school_id=school_id, request=self.request
            )
        else:
            if pk:
                ...
                return ClubForm(
                    instance=instance, school_id=school_id, pk=pk, request=self.request
                )
...
class MeetingFormView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class CommunityInitiativeListView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    FilterView,
    ExportMixin,
...
class CommunityInitiativeFormView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    def get_form(self, form_class=None):
        ...
        if self.request.method == "POST":
            return CommunityInitiativeForm(
                self.request.POST, pk=pk, school_id=school_id, request=self.request
            )
        else:
            if pk:
            ...
            return CommunityInitiativeForm(
                school_id=school_id, pk=pk, request=self.request
            )
...
class HealthVisitFormView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
def export_school_background(request):
    try:
        ...
        partner_name = (
            user.partner.name if hasattr(user, "partner") and user.partner else ""
        )
        ...
        qs_community_initiative = CommunityInitiative.objects.filter(
            school_id__in=school_ids
        ).order_by("-id")
        qs_health_visit = HealthVisit.objects.filter(school_id__in=school_ids).order_by(
            "-id"
        )
        ...
        with zipfile.ZipFile(zip_output, "w") as zf:
            ...
            add_csv_to_zip(
                zf, "qs_community_initiative_data.csv", qs_community_initiative
            )
