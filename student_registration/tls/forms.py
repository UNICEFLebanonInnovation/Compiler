...
class MainForm(forms.ModelForm):
    ...
    YEARS = list(((str(x), x) for x in range(2005, Child.CURRENT_YEAR + 1)))
    YEARS.insert(0, ('', '---------'))
    ...
    source_of_identification = forms.ChoiceField(
        label=_("Source of referral of the child to TLS"),
        widget=forms.Select,
        required=True,
    ...
    def __init__(self, *args, **kwargs):
        ...
        form_action = reverse('tls:child_add')
        if instance:
            display_registry = ' d-none'
            form_action = reverse('tls:child_edit', kwargs={'pk': instance.id})
