...
from django.views.generic import DetailView, FormView, ListView, RedirectView, TemplateView, UpdateView, View
from openpyxl import Workbook
from rest_framework import mixins, permissions, viewsets

from .models import Child, HouseHold, OutreachCaregiver, OutreachChild
from .serializers import ChildSerializer, HouseHoldSerializer

MAX_CHUNKS = 5


def outreach_import_data(request):
    last_loaded_identifier = OutreachCaregiver.objects.filter(
        form_id="aLkUps4YnMc43ATvTr9JB3"
    ).aggregate(Max("u_id"))["u_id__max"]
    if last_loaded_identifier is None:
        last_loaded_identifier = 0
    # last_loaded_identifier = 0
    last_loaded_identifier_str = str(last_loaded_identifier)
    url = (
        "https://eu.kobotoolbox.org/api/v2/assets/aLkUps4YnMc43ATvTr9JB3/data.json?sort=%7B%22_id%22%3A+1%7D&query=%7B%22_id%22%3A+%7B%22%24gt%22%3A+"
        + last_loaded_identifier_str
        + "%7D%7D"
    )
    ...
    for record in data["results"]:
        try:
            ...
            record_value(
                caregiver, "cadaster_other_specify", record, "cadaster_other_specify"
            )
            ...
            record_value(
                caregiver, "caregiver_nationality", record, "caregiver_nationality"
            )
            record_value(
                caregiver,
                "caregiver_nationality_other",
                record,
                "caregiver_nationality_other",
            )
            record_value(
                caregiver, "caregiver_first_name", record, "caregiver_first_name"
            )
            record_value(
                caregiver, "caregiver_father_name", record, "caregiver_father_name"
            )
            record_value(caregiver, "caregiver_mother_name", record, "caregiver_mother")
            record_value(
                caregiver, "caregiver_last_name", record, "caregiver_last_name"
            )
            ...
            record_value(
                caregiver, "caregiver_personal_id", record, "caretaker_personal_id"
            )
            record_value(
                caregiver, "father_education_level", record, "father_education_level"
            )
            record_value(
                caregiver, "mother_education_level", record, "mother_education_level"
            )
            record_value(
                caregiver, "other_education_level", record, "other_education_level"
            )
            ...
            if "DC" in record:
                try:
                    for student in record["DC"]:
                        ...
                        record_value(
                            st,
                            "dropout_reason_other",
                            student,
                            "DC/dropout_reason_other",
                        )
...
class OutreachPage(LoginRequiredMixin, TemplateView):
...
def outreach_unregistered_export_info(request):
    cursor = connection.cursor()
    cursor.execute(
        "SELECT COUNT(*) FROM vw_outreach_not_registered WHERE caregiver_id > 0"
    )
...
def outreach_unregistered_export_data(request, part):
    ...
    vw_data = "SELECT * FROM vw_outreach_not_registered WHERE caregiver_id > 0 LIMIT {} OFFSET {}".format(
        records_per_chunk, offset
    )
    cursor.execute(vw_data)
    data = cursor.fetchall()
    headers = [col[0] for col in cursor.description]
    ...
    response["Content-Disposition"] = (
        "attachment; filename=outreach_not_registered_data_part_{}.xlsx".format(part)
    )
    return response
