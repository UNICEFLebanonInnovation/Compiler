...
class ExportView(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        items = Child.objects.order_by('nationality_id').values_list('nationality_id').distinct()
        ...
        content = []
        for item in items:
            qs = queryset.filter(nationality_id=item[0])
            nbr_cls = qs.filter(
                ...
                Q(disability_type__contains=['Other'])).count()
...
class Export2View(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # items = Child.objects.order_by('calculated_age').values_list('calculated_age').distinct()
