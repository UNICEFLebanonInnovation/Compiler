...
import tablib
from django.contrib.auth.mixins import GroupRequiredMixin, LoginRequiredMixin
...
from .filters import OutreachFilter, PostTestFilter, PreTestAllFilter, PreTestFilter, SchoolFilter
from .forms import OutreachForm, PostTestGradingForm, PreTestAllForm, PreTestForm, PreTestGradingForm, RegistrationForm
from .models import ALPRound, Outreach
from .serializers import GeneralSerializer, GradingSerializer, OutreachSerializer, OutreachSmallSerializer
from .tables import BootstrapTable, OutreachTable, PostTestTable, PreTestTable, SchoolTable
    ...
    def get_queryset(self):
        ...
        if self.request.user.school_id:
            ...
            return self.queryset.filter(
                school_id=self.request.user.school_id, alp_round=alp_round
            )
...
class AddView(LoginRequiredMixin, GroupRequiredMixin, FormView):
    ...
    group_required = ["ALP_SCHOOL", "ALP_DIRECTOR"]
...
class SchoolView(
    ...
    def get_queryset(self):
        ...
        return Outreach.objects.filter(
            alp_round=alp_round, school=self.request.user.school_id
        )


class PreTestAllView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    FilterView,
    ExportMixin,
    ...
    group_required = ["TEST_MANAGER", "CERD"]
...
class PreTestView(
    ...
    def get_queryset(self):
        ...
        alp_round = ALPRound.objects.get(current_pre_test=True)
        return Outreach.objects.filter(
            alp_round=alp_round, school_id=self.request.user.school
        )


class PreTestAddView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class PreTestAddAllView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class PreTestEditView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class PreTestGradingView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class PostTestView(
    ...
    def get_queryset(self):
        ...
        return Outreach.objects.filter(
            alp_round=alp_round, registered_in_level__isnull=False
        )


class PostTestGradingView(LoginRequiredMixin, GroupRequiredMixin, FormView):
...
class ExportViewSet(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if school:
            response = export_alp(
                {current_type: "true", "school": school}, return_data=True
            )
...
class ExportBySchoolView(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        schools = (
            self.queryset.filter(alp_round=alp_round, registered_in_level__isnull=False)
            .values_list(
                "school",
                "school__number",
                "school__name",
        ...
        for school in schools:
            nbr = self.model.objects.filter(
                school=school[0], alp_round=alp_round, registered_in_level__isnull=False
            ).count()
